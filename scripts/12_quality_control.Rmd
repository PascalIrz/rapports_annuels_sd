...
site  <- as.data.frame(codes_sandre) %>% count(codes_sandre) %>% filter(n > 1) %>% pull(codes_sandre)
...
# mon_double <- "04178486"
# 
# codes_sandres_new_names <- str_sub(new_names, 1, 8)
# 
# length(codes_sandre)
# length(codes_sandres_new_names)
# 
# setdiff(codes_sandre, codes_sandres_new_names)




# test <- data.frame(file_names = sort(file_names),
#                    code_sandre = sort(codes_sandre)) %>% 
#   mutate(test = which(str_detect(test$file_names, codes_sandres_new_names)))
