...
mesure_individuelle <- misc_nom_dernier_fichier(repertoire = "../raw_data",
                                         pattern = "^mei")

load(mesure_individuelle)
```
...
# les stations qui ne sont pas échantillonées sur les deux dernières années sont exclues
reg_ope <- reg_data %>% 
...
mes_pre <- reg_data %>% filter(ope_id %in% mes_ope_reg) %>% pull(pre_id) %>% unique()

temps_passages <- prelevement_elementaire %>% 
  filter(pre_id %in% mes_pre) %>% 
  group_by(ope_id = pre_ope_id) %>% 
  summarise(duree_peche = sum(pre_duree, na.rm =T))

reg_ope_env <- reg_ope_env %>% 
  left_join(temps_passages) %>% 
  mutate(odp_duree_peche = ifelse(odp_duree_peche == 0,
                                  duree_peche,
                                  odp_duree_peche))
