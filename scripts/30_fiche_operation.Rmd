...
```{r, fig.width = 9, dpi = 300, fig.height = 4, fig.cap = "Variabilité interannuelle des caractéristiques environnementales de la station."}
reg_ope_env %>% 
...
```{r, fig.width = 9, dpi = 300, fig.height = 4, fig.cap = "Variabilité interannuelle des paramètres de pêche."}
reg_ope_env %>% 
  filter(
    pop_id == mon_pop,
    (variable %in% vars_peche)) %>% 
...
```{r, fig.height = 3, fig.cap="Variation interannuelle de l'IPR sur la station."}
...
```{r, fig.width = 8, fig.height = 5.5, fig.cap="Variation interannuelle des métriques IPR sur la station. L'indice agrégé étant la somme de 7 métriques, les seuils de qualité IPR sont ici divisés par 7 pour chacune des métriques."}
reg_ope_metrique %>%
  filter(pop_id == mon_pop) %>%
  mutate(
...
```{r, fig.height = hauteur_fig, fig.width = 10, fig.cap = "Variabilité interannuelle des densités par espèce, en nombre d'individus pour 1000 m². NB L'échelle des ordonnées varie d'espèce en espèce."}
mes_especes <- data %>% 
...
data %>% 
  ...
  gg_temp_abondance_groupe(var_espece = esp_nom_commun,
                  var_abondance = densite,
                  nb_colonnes = 5)
```

```{r, fig.height = 1 + hauteur_fig * 0.6, fig.cap = "Dynamique interannuelle du peuplement. La superficie de chaque point est proportionnelle à l'effectif capturé. Le protocole d'échantillonnage employé est indiqué sous le graphique des abondances. Les taxons qui contribuent au calcul de l'IPR sont indiquées en caractères gras."}
test <- reg_ope_capt %>% 
  filter(pop_id == mon_pop,
         effectif > 0) %>% 
...
```{r, fig.height = 1 + richesse / 5, fig.cap = "Effectif capturé par espèce."}
data %>% 
...
```{r, fig.height = 4.5, dpi = 300, fig.cap=paste0("Présence / absence des espèces en ", mon_annee, ", en regard de leur probabilité de présence modélisée sur des sites de référence (d'après Oberdorff et al. 2001). Si la station est en bon état, les espèces avec les plus fortes probabilités de présence (en haut du graphique) sont observées in situ, donc leur barre est de couleur verte, tandis celles en bas du graphique sont attendues absentes, donc de couleur marron.")}
ggplot(data = test,
